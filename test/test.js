...
describe( 'storage.json', function(){
    ...
    describe( '#parse()', function(){
        it( 'should return an object containing a "value" property, ONLY if not passed a valid string', function(){
            storage.parse( false ).value.should.equal( false );
            storage.parse( true ).value.should.equal( true );
            var isNull = storage.parse( null ).value === null;

            isNull.should.equal( true );
        });

        it( 'should parse file paths which prefix the filename separated by a colon', function(){
            storage.parse( 'root:filename' ).should.deep.equal({ name: 'filename.json', path: storage.directory.app(), prop: '', file: 'root:filename.json' });
        });
    ...
    describe( '#ignore()', function(){
        ...
        it( 'should append the defaults filename into the .npmignore files', function(){
            storage.ignore( '.npmignore' );
    ...
    describe( '#setup()', function(){
        ...
        beforeEach( function(){
            originals = {
                defaultsFileName: storage.defaultsFileName,
                rootFileName: storage.rootFileName,
                moduleFileName: storage.moduleFileName,
            ...
            test_options = {
                npmIgnore: true,
                gitIgnore: true,
                defaultsFileName: 'defaults.changed',
                rootFileName: 'config.changed',
                moduleFileName: 'app.changed',
        ...
        it( 'should use the storage.json specific properties, to alter the instance settings', function(){
            ...
            storage.defaultsFileName.should.equal( 'defaults.changed' );
            storage.rootFileName.should.equal( 'config.changed' );
            storage.moduleFileName.should.equal( 'app.changed' );
    ...
    describe( '#config()', function(){
        ...
        afterEach( function(){
            ...
            storage.delete( storage.moduleFileName );
        });
        ...
        describe( 'arg:options (default:{})', function(){
            ...
            it( 'should accept an object, and bootstrap the instance with its values', function(){
                storage.config( 'config.json', { moduleFileName: 'config.json' });
                storage.moduleFileName.should.equal( 'config.json' );
            });

            it( 'should accept the "defaults" object, if the "options" object was passed as argument 1', function(){
                storage.config({ moduleFileName: 'config.json' }, { host: '10.0.0.1' });
                storage.read( storage.moduleFileName ).should.deep.equal({ host: '10.0.0.1' });
            });
        });
        ...
        describe( 'arg:defaults (default:{})', function(){
            ...
            it( 'should accept an object, and inject its properties into the module file ( if previously undefined )', function(){
                storage.config( 'config', { moduleFileName: 'config.json', host: '10.0.0.2' }, { host: '10.0.0.1', unique: 'unique' });
                storage.read( storage.moduleFileName ).should.deep.equal({ host: '10.0.0.2', unique: 'unique' });
            });
        });
        ...
        it( 'should generate a file within the module directory, if it does not exist ( filename variable: moduleFileName )', function(){
            storage.exists( 'app:config.json' ).should.equal( false );
            storage.config({ moduleFileName: 'config.json' });
        ...
        it( 'should generate ONLY the root file, if module and root paths are identical', function(){
            ...
            storage.config({ moduleFileName: 'module', rootFileName: 'config' });
        ...
        it( 'should NOT generate the root file if options.rootFileName is set to false', function(){
            if( storage.exists( 'root:config' )) console.log( 'EXISTS BEFORE:', storage.read( 'root:config' ));
        ...
        it( 'should NOT generate the module file if options.moduleFileName is set to false', function(){
            storage.exists( 'app:config.json' ).should.equal( false );
            storage.config({ moduleFileName: false });
            storage.exists( 'app:config.json' ).should.equal( false );
        });

        it( 'should inject default properties into the module file, from the "defaults" object', function(){
            storage.config( 'config.json', { moduleFileName: 'config.json', defaults: { host: '10.0.0.1' }});
            storage.read( storage.moduleFileName ).should.deep.equal({ host: '10.0.0.1' });
        });

        it( 'should NOT overwrite existing properties with default values, in the module file', function(){
            storage.config( 'config', { moduleFileName: 'config.json', host: '10.0.0.2' });
            storage.config( 'config', { moduleFileName: 'config.json', defaults: { host: '10.0.0.1' }});
            storage.read( storage.moduleFileName ).should.deep.equal({ host: '10.0.0.2' });
        });

        it( 'should override default properties within the module file, when assigned within options', function(){
            storage.config({ moduleFileName: 'config.json', host: '10.0.0.2', defaults: { host: '10.0.0.1' }});
            storage.read( storage.moduleFileName ).should.deep.equal({ host: '10.0.0.2' });
        });

        it( 'should copy existing properties into the root file, from the module file, if both files exist', function(){
            storage.config( 'config', { moduleFileName: 'config.json', host: '10.0.0.2', defaults: { host: '10.0.0.1' }});
        ...
        it( 'should inject properties into the root file, if the module file does not exist', function(){
            storage.config( 'config', { moduleFileName: false, defaults: { host: '10.0.0.1' }});
    ...
    describe( '#include()', function(){
        it( 'should remove references to config filenames from the .npmignore files', function(){
            ...
            storage.read( '.npmignore' ).indexOf( storage.defaultsFileName ).should.equal( -1 );
            storage.read( '.npmignore' ).indexOf( storage.rootFileName ).should.equal( -1 );
            storage.read( '.npmignore' ).indexOf( storage.moduleFileName ).should.equal( -1 );
        });
